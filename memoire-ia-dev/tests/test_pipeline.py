...
def test_preserves_pull_request_checkpoint_position_after_forbidden_response(monkeypatch) -> None:
    from memoire_ia_dev import prs
    ...
    class Response:
        ...
        def raise_for_status(self) -> None:
            raise AssertionError("A 403 response should end collection cleanly")

    monkeypatch.setitem(__import__("sys").modules, "requests", type("Requests", (), {"get": lambda *_args, **_kwargs: Response()})())

    result, complete, next_page = prs.get_pull_requests("owner", "repo", start_page=386)

    assert result.empty
    assert not complete
    assert next_page == 386
